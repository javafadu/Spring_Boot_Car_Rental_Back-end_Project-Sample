...
public class ReservationService {
    ...
    @Transactional(readOnly = true)
    // image log larini da getirebilmesi icin @Transactional annotation eklememiz gerekiyor
    public ReservationDTO findByIdAndUserId(Long id, Long userId){
        // check user if exist
        User user = userRepository.findById(userId).orElseThrow(()-> new ResourceNotFoundException(String.format(ErrorMessage.RESOURCE_NOT_FOUND_MESSAGE,userId)));

        return reservationRepository.findByIdAndUserId(id,user).orElseThrow(()->new ResourceNotFoundException(String.format(ErrorMessage.RESOURCE_NOT_FOUND_MESSAGE,id)));
